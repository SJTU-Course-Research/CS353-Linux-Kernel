...
static char *encry_file_name = "NULL";
static char *addex_file_name = "NULL";

module_param(hided_file_name,charp,S_IRUGO);
module_param(encry_file_name,charp,S_IRUGO);
module_param(addex_file_name,charp,S_IRUGO);
...
static int romfs_readpage(struct file *file, struct page *page)
{
	...
	void *buf;
	int ret;
	char *filename = file->f_dentry->d_name.name;
	...
	if(strcmp(filename,encry_file_name)==0){
		int i=0;
		for(i = 0;i<fillsize-1;++i)		*((char *)buf + i)='*';
	}
